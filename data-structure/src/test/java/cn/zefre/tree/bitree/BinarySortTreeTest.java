...
public class BinarySortTreeTest {
    ...
    public void testInOrder() {
        List<Integer> integers = bitSortTree.sequence(OrderEnum.IN_ORDER);
    ...
    public void testFind() {
        BinarySortTree.BSTNode<Integer> node = bitSortTree.find(6);
    ...
    @Test
    public void testGet() {
        BinarySortTree.BSTNode<Integer> node = bitSortTree.get(6);
        Assert.assertEquals(Integer.valueOf(6), node.data);
        Assert.assertNull(node.left);
        Assert.assertEquals(Integer.valueOf(14), node.right.data);
    }
    ...
    public void testInsert() {
        ...
        List<Integer> expected = Arrays.asList(6, 14, 18, 24, 27, 28, 32, 34, 39, 42, 48, 64, 66);
        Assert.assertEquals(expected, bsTree.sequence(OrderEnum.IN_ORDER));
    }

    @Test
    public void testRemove() {
        BinarySortTree<Integer> binarySortTree = new BinarySortTree<>();
        for (Integer num : bitSortTree.sequence(OrderEnum.SEQUENCE)) {
        ...
        // 测试删除完成后树结构
        Assert.assertEquals(Arrays.asList(6, 14, 24, 27, 32, 42, 48, 66), binarySortTree.sequence(OrderEnum.IN_ORDER));
    ...
    public void testDelete() {
        ...
        // 测试删除完成后树结构
        Assert.assertEquals(Arrays.asList(39, 28, 64, 6, 32, 48, 14), binarySortTree.sequence(OrderEnum.SEQUENCE));
